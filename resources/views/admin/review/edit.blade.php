                        ...
                        <form action="{{ route('admin.reviews.update', $review?->id) }}" method="POST"
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control {{ \App\Helpers\hasError($errors, 'name') }}"
                                    value="{{ old('name', $review?->name) }}" name="name">
                                <x-input-error :messages="$errors->get('name')" class="mt-2" />
                            </div>
                            <div class="form-group">
                                <label for="">Title</label>
                                <input type="text" class="form-control {{ \App\Helpers\hasError($errors, 'title') }}"
                                    value="{{ old('title', $review?->title) }}" name="title">
                                <x-input-error :messages="$errors->get('title')" class="mt-2" />
                            </div>
                            <div class="form-group">
                                <label for="">Review</label>
                                <textarea class="{{ \App\Helpers\hasError($errors, 'review') }} form-control" name="review">{{ $review?->review }}</textarea>
                            ...
                            <div class="form-group">
                                ...
                                <select name="rating" class="form-control {{ \App\Helpers\hasError($errors, 'rating') }}">
                                    @for ($i = 1; $i <= 5; $i++)
                                        <option @selected($review?->rating === $i) value="{{ $i }}">
                                            {{ $i }}</option>
                                    @endfor
                                </select>
